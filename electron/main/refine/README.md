...
- `service.ts` - 无状态 `RefineService`，每次调用都读取最新 `llmRefine` 配置，使用固定 transcript 包装与抗注入 prompt 执行润色和连接测试。
...
- `__tests__/service.test.ts` - 覆盖 endpoint 规范化、抗注入 prompt / transcript 包装、错误映射与连接测试。
